...
class UCBEmitter(Emitter):
    ...
    def emit(
        ...
        if n_variation > 0:
            x1_indices = [repertoire.ucb_selection() for _ in range(n_variation)]
            x2_indices = [repertoire.ucb_selection() for _ in range(n_variation)]
            x1 = repertoire.sample_by_indices(x1_indices)
            x2 = repertoire.sample_by_indices(x2_indices)
            x_variation, random_key = self._variation_fn(x1, x2, random_key)

        if n_mutation > 0:
            x1_indices = [repertoire.ucb_selection() for _ in range(n_mutation)]
            x1 = repertoire.sample_by_indices(x1_indices)
